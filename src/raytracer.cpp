...
Ray PhongRaytracer::calculate_refraction_ray(Ray& ray, Intersection& hit_info)
{
  ...
  Vec3 r_out_perp = (hit_info.normal + ray.direction * cos_theta) *
                    hit_info.object->material.refractive_index;
  Vec3 r_out_parallel =
      hit_info.normal * -sqrt(fabs(1.0 - r_out_perp.dot(r_out_perp)));
